#!/usr/bin/env python3
"""Diagnose W4A4 Hessian-sensitive blocks and draw global/per-layer curves.

输入:
  1. 预训练模型目录, 由 --model-dir 控制, 默认读取
     /root/autodl-tmp/model/Qwen3-4B-Instruct-2507。
  2. SmoothQuant 分组/scale 文件, 由 --smooth-groups 控制。
  3. WikiText2 校准文本, 默认走项目内加载逻辑; 可用 --local-wikitext2-dir
     指向本地数据。

主要参数:
  --module-filter / --layer-filter 控制分析范围, 例如只看 q_proj/v_proj/gate_proj
  或 0-35 层。
  --block-rows / --block-cols 控制 block 形状, 默认 16 x 128。
  --groupsize / --weight-bits / --act-order 控制 GPTQ 风格量化。
  --plot-observation23 打开 Observation 2/3 selection curve。
  --obs23-max-fraction 控制 selection curve 横轴最大 block 比例。
  --obs23-weight-outlier-mode 控制 weight-outlier baseline 用 max/mean weight
  还是 residual。
  --obs23-format / --obs23-dpi 控制曲线图保存格式和分辨率。
...
画出的图像:
  output-dir/plots/concentration_curves.png:
    全局 top blocks 捕获 exact output error mass 的集中度。
  output-dir/plots/repair_curves.png:
    全局不同打分方法选 block 后的剩余误差比例。
  output-dir/plots/score_correlation.png:
    全局 Hessian/Frobenius/channel 分数与 exact output score 的相关散点图。
  output-dir/plots/heatmaps/*.png:
    高误差模块的 exact output score 与 Hessian diag score heatmap。
  output-dir/plots/observation23/observation23_selection_curve.png|pdf:
    全局 Observation 2/3 selection curve。
  output-dir/plots/observation23/by_layer/layer_XX_observation23_selection_curve.png|pdf:
    每一层单独的 selection curve, 用来比较不同层的 block 可识别性。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable the Observation 2 selection curve figure.",
    )
    parser.add_argument(
        "--obs23-max-fraction",
        type=float,
        default=0.20,
        help="Maximum selected block fraction shown in the Observation 2 curve plot.",
    )
    parser.add_argument(
        "--obs23-dpi",
        type=int,
        default=300,
        help="DPI for saving the Observation 2 curve plot.",
    )
    parser.add_argument(
        "--obs23-format",
        choices=["png", "pdf", "both"],
        default="both",
        help="File format for the Observation 2 curve plot.",
    )
    parser.add_argument(
        "--obs23-weight-outlier-mode",
        choices=["max_abs_weight", "mean_abs_weight", "max_abs_residual"],
        default="max_abs_weight",
        help="Weight-outlier baseline score used in the Observation 2 curve plot.",
    )
    parser.add_argument(
        "--obs23-title",
        type=str,
        default="Output-critical blocks are sparse and cannot be identified by weight outliers alone",
        help="Title for the Observation 2 curve plot.",
    )
    ...
    if not (0.0 < args.obs23_max_fraction <= 1.0):
        parser.error("--obs23-max-fraction must be in (0, 1].")
    if args.obs23_dpi <= 0:
        parser.error("--obs23-dpi must be positive.")
    return args
...
def aggregate_rows_max(row_values: torch.Tensor, block_rows: int, nrow: int) -> torch.Tensor:
    total_rows = nrow * block_rows
    if row_values.numel() < total_rows:
        pad = torch.zeros(total_rows - row_values.numel(), device=row_values.device, dtype=row_values.dtype)
        row_values = torch.cat([row_values, pad], dim=0)
    return row_values.view(nrow, block_rows).amax(dim=1)


def aggregate_rows_mean(row_values: torch.Tensor, block_rows: int, nrow: int) -> torch.Tensor:
    sums = aggregate_rows(row_values, block_rows, nrow)
    actual_rows = int(row_values.numel())
    counts = [
        max(0, min((br + 1) * block_rows, actual_rows) - br * block_rows)
        for br in range(nrow)
    ]
    count_tensor = torch.as_tensor(counts, device=row_values.device, dtype=row_values.dtype).clamp_min(1)
    return sums / count_tensor
...
def analyze_module(
    ...
    weight_outlier_scores = torch.zeros_like(frob_scores)
    ...
    for bc in range(ncol):
        ...
        if args.obs23_weight_outlier_mode == "max_abs_weight":
            weight_outlier_row = w_for[:, c0:c1].abs().float().amax(dim=1)
            weight_outlier_scores[:, bc] = aggregate_rows_max(weight_outlier_row, block_rows, nrow)
        elif args.obs23_weight_outlier_mode == "mean_abs_weight":
            weight_outlier_row = w_for[:, c0:c1].abs().float().mean(dim=1)
            weight_outlier_scores[:, bc] = aggregate_rows_mean(weight_outlier_row, block_rows, nrow)
        elif args.obs23_weight_outlier_mode == "max_abs_residual":
            weight_outlier_row = r_col.abs().amax(dim=1)
            weight_outlier_scores[:, bc] = aggregate_rows_max(weight_outlier_row, block_rows, nrow)
        else:
            raise ValueError(f"Unsupported weight outlier mode: {args.obs23_weight_outlier_mode}")
    ...
    weight_outlier_np = tensor_matrix_to_numpy(weight_outlier_scores)
    ...
    w_pearson, w_spearman = correlations(exact_np.reshape(-1), weight_outlier_np.reshape(-1))
    ...
    summary = {
        ...
        "total_weight_outlier_score": finite_float(float(weight_outlier_np.sum())),
        ...
        "corr_weight_outlier_pearson": finite_float(w_pearson),
        "corr_weight_outlier_spearman": finite_float(w_spearman),
    }
            ...
            block_rows_json.append(
                {
                    ...
                    "weight_outlier_score": finite_float(weight_outlier_np[br, bc]),
                }
            )
    ...
    matrices = {
        ...
        "channel": channel_np,
        "delta": delta_np,
        "weight_outlier": weight_outlier_np,
    }

    del weight, scale, w_s, w_for, q0, residual, frob_scores, hdiag_scores
    del exact_hessian_scores, exact_output_scores, channel_scores, delta_scores, weight_outlier_scores
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def write_module_summary(path: Path, rows: list[dict]) -> None:
    ...
    base_fields = [
        ...
        "corr_delta_pearson",
        "corr_delta_spearman",
        "corr_weight_outlier_pearson",
        "corr_weight_outlier_spearman",
    ]
...
def observation23_fraction_grid(max_fraction: float) -> list[float]:
    max_fraction = min(1.0, max(0.0, float(max_fraction)))
    grid = np.linspace(0.0, max_fraction, 201, dtype=np.float64)
    extras = [0.10] if max_fraction >= 0.10 else []
    values = np.unique(np.round(np.concatenate([grid, np.asarray(extras, dtype=np.float64)]), 10))
    return [float(value) for value in values if value <= max_fraction + 1e-12]


def captured_mass_at(
    repair_rows: list[dict],
    method: str,
    fraction: float,
) -> float | None:
    for row in repair_rows:
        if row.get("method") == method and math.isclose(float(row.get("fraction", -1.0)), fraction, abs_tol=1e-10):
            return finite_float(row.get("captured_mass"))
    return None


def summarize_observation23(
    global_arrays: dict[str, list[np.ndarray]],
    max_fraction: float,
    random_trials: int,
    seed: int,
) -> dict:
    required = ("exact", "weight_outlier", "frob", "channel")
    arrays: dict[str, np.ndarray] = {}
    for key in required:
        values = global_arrays.get(key, [])
        arrays[key] = np.concatenate(values).reshape(-1).astype(np.float64) if values else np.empty(0, dtype=np.float64)

    sizes = {key: value.size for key, value in arrays.items()}
    if len(set(sizes.values())) > 1:
        raise ValueError(f"Observation 2 arrays must have matching sizes, got {sizes}")

    exact = np.nan_to_num(arrays["exact"], nan=0.0, posinf=0.0, neginf=0.0)
    exact = np.maximum(exact, 0.0)
    total_exact = float(exact.sum())
    if exact.size == 0:
        return {
            "total_blocks": 0,
            "total_exact_output_mass": 0.0,
            "oracle_captured_at_0.10": None,
            "weight_outlier_captured_at_0.10": None,
            "residual_norm_captured_at_0.10": None,
            "channel_only_captured_at_0.10": None,
            "random_captured_at_0.10": None,
            "repair_rows": [],
        }

    rankings = {
        "exact_oracle": exact,
        "weight_outlier": arrays["weight_outlier"],
        "residual_norm": arrays["frob"],
        "channel_only": arrays["channel"],
    }
    repair_rows = repair_curve(
        exact=exact,
        rankings=rankings,
        fracs=observation23_fraction_grid(max_fraction),
        random_trials=random_trials,
        seed=seed,
    )
    rows_at_10 = repair_curve(
        exact=exact,
        rankings=rankings,
        fracs=[0.10],
        random_trials=random_trials,
    ...
    return {
        "total_blocks": int(exact.size),
        "total_exact_output_mass": finite_float(total_exact),
        "oracle_captured_at_0.10": captured_mass_at(rows_at_10, "exact_oracle", 0.10),
        "weight_outlier_captured_at_0.10": captured_mass_at(rows_at_10, "weight_outlier", 0.10),
        "residual_norm_captured_at_0.10": captured_mass_at(rows_at_10, "residual_norm", 0.10),
        "channel_only_captured_at_0.10": captured_mass_at(rows_at_10, "channel_only", 0.10),
        "random_captured_at_0.10": captured_mass_at(rows_at_10, "random", 0.10),
        "repair_rows": repair_rows,
    }


def new_observation23_arrays() -> dict[str, list[np.ndarray]]:
    return {
        "exact": [],
        "weight_outlier": [],
        "frob": [],
...
def plot_observation23_curves(
    ...
    dpi: int,
    file_format: str,
    plot_subdir: str = "observation23",
    filename_stem: str = "observation23_selection_curve",
) -> None:
    plt = setup_matplotlib()
    if plt is None:
        print("[warn] matplotlib is unavailable; skipping Observation 2 curve plot.", flush=True)
        return
    if not repair_rows:
        print("[warn] no Observation 2 repair rows; skipping Observation 2 curve plot.", flush=True)
        return

    plots_dir = output_dir / "plots" / plot_subdir
    plots_dir.mkdir(parents=True, exist_ok=True)

    fig, ax = plt.subplots(figsize=(7.2, 5.0), constrained_layout=True)
    method_styles = [
        ("exact_oracle", "Exact output oracle", "black", "-", 2.2),
        ("weight_outlier", "Weight outlier", "#E69F00", "--", 2.0),
        ("residual_norm", "Residual norm", "#0072B2", "--", 2.0),
        ("channel_only", "Channel-only", "#009E73", "-.", 2.0),
        ("random", "Random", "#7A7A7A", ":", 2.0),
    ]
    for method, label, color, linestyle, linewidth in method_styles:
        rows = sorted(
            (row for row in repair_rows if row["method"] == method),
        ...
        y = np.array(
            [
                np.nan if row.get("captured_mass") is None else 100.0 * float(row["captured_mass"])
                for row in rows
            ],
            dtype=np.float64,
        )
    ...
    max_x = 100.0 * float(max_fraction)
    if max_x >= 10.0:
        ax.axvline(10.0, color="black", linestyle="--", linewidth=1.2, alpha=0.45)
        oracle_at_10 = captured_mass_at(repair_rows, "exact_oracle", 0.10)
        if oracle_at_10 is not None:
            oracle_pct = 100.0 * oracle_at_10
            ax.annotate(
                f"10% blocks capture {oracle_pct:.1f}% oracle mass",
                xy=(10.0, oracle_pct),
                xytext=(min(max_x - 0.5, 11.2), min(96.0, max(12.0, oracle_pct + 7.0))),
                textcoords="data",
                fontsize=9,
                ha="left",
                va="center",
                arrowprops={"arrowstyle": "-", "color": "black", "alpha": 0.45, "linewidth": 1.0},
            )
    ...
    for fmt in formats:
        fig.savefig(plots_dir / f"{filename_stem}.{fmt}", dpi=dpi)
    plt.close(fig)
...
def main() -> None:
    ...
    layer_observation23_arrays: dict[int, dict[str, list[np.ndarray]]] = {}
        ...
        for cur_group_items, input_cache in group_batches:
            for group_name, _group_prefix, module_names in cur_group_items:
                ...
                for module_name in module_names:
                    ...
                    global_arrays["weight_outlier"].append(matrices["weight_outlier"].reshape(-1))
                    if summary["layer_id"] is not None:
                        layer_id = int(summary["layer_id"])
                        layer_arrays = layer_observation23_arrays.setdefault(layer_id, new_observation23_arrays())
                        layer_arrays["exact"].append(matrices["exact"].reshape(-1))
                        layer_arrays["weight_outlier"].append(matrices["weight_outlier"].reshape(-1))
                        layer_arrays["frob"].append(matrices["frob"].reshape(-1))
                        layer_arrays["channel"].append(matrices["channel"].reshape(-1))
    ...
    observation23_summary_path = None
    if args.plot_observation23:
        observation23_summary = summarize_observation23(
            global_arrays=global_arrays,
            max_fraction=args.obs23_max_fraction,
            random_trials=args.random_trials,
            seed=args.seed,
        )
        observation23_summary.update(
            {
                "title": args.obs23_title,
                "max_fraction": args.obs23_max_fraction,
                "weight_outlier_mode": args.obs23_weight_outlier_mode,
                "random_trials": args.random_trials,
                "seed": args.seed,
            }
        )
        per_layer_summaries = {}
        for layer_id, layer_arrays in sorted(layer_observation23_arrays.items()):
            layer_summary = summarize_observation23(
                global_arrays=layer_arrays,
                max_fraction=args.obs23_max_fraction,
                random_trials=args.random_trials,
                seed=args.seed + layer_id * 1009,
            )
            layer_summary.update(
                {
                    "layer_id": layer_id,
                    "title": f"Layer {layer_id}: {args.obs23_title}",
                    "max_fraction": args.obs23_max_fraction,
                    "weight_outlier_mode": args.obs23_weight_outlier_mode,
                    "random_trials": args.random_trials,
                    "seed": args.seed + layer_id * 1009,
            ...
            per_layer_summaries[str(layer_id)] = layer_summary
        observation23_summary["per_layer"] = per_layer_summaries
        observation23_summary_path = args.output_dir / "observation23_summary.json"
        observation23_summary_path.write_text(
            json.dumps(observation23_summary, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        plot_observation23_curves(
            output_dir=args.output_dir,
            repair_rows=observation23_summary["repair_rows"],
            max_fraction=args.obs23_max_fraction,
            title=args.obs23_title,
            dpi=args.obs23_dpi,
            file_format=args.obs23_format,
        )
        for layer_id, layer_summary in sorted(per_layer_summaries.items(), key=lambda item: int(item[0])):
            plot_observation23_curves(
                output_dir=args.output_dir,
                repair_rows=layer_summary["repair_rows"],
                max_fraction=args.obs23_max_fraction,
                title=layer_summary["title"],
                dpi=args.obs23_dpi,
                file_format=args.obs23_format,
                plot_subdir="observation23/by_layer",
                filename_stem=f"layer_{int(layer_id):02d}_observation23_selection_curve",
            )
    ...
    if observation23_summary_path is not None:
        print("[done] observation23 summary:", observation23_summary_path, flush=True)
    if args.save_per_block:
