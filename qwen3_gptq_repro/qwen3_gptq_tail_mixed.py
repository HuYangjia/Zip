...
def load_custom_model_class(modeling_file: str):
    ...
    module_name = "transformers.models.qwen3.modeling_qwen3_custom_tail_mixed"
...
@torch.no_grad()
def quantize_tail_int8_inplace(linear: nn.Linear, tail_ratio: float, eps: float = 1e-8):
    ...
    tail_cols = min(in_features - 1, tail_cols)
    tail_start = in_features - tail_cols
    tail_slice = w[:, tail_start:].float()

    max_abs = tail_slice.abs().amax(dim=1, keepdim=True)
    scales = torch.clamp(max_abs / 127.0, min=eps)
    q = torch.clamp(torch.round(tail_slice / scales), -127, 127).to(torch.int8)
    dq = q.float() * scales
    err = (dq - tail_slice).abs().mean().item()
    w[:, tail_start:] = dq.to(dtype=w.dtype)

    return {
        "in_features": in_features,
        "tail_start": tail_start,
        "tail_cols": tail_cols,
        "tail_ratio": float(tail_cols / in_features),
        "tail_quant": "int8",
        "tail_scale_min": float(scales.min().item()),
        "tail_scale_max": float(scales.max().item()),
        "tail_abs_mean_error": err,
    }
...
def qwen3_sequential(
    ...
    enable_tail_mixed: bool,
    tail_ratio: float,
    tail_quant: str,
):
    print("Starting sequential GPTQ (Qwen3 tail-mixed) ...")

    if enable_tail_mixed and tail_quant != "int8":
        raise ValueError(f"unsupported --tail-quant: {tail_quant}")
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                if enable_tail_mixed:
                    tail_info = quantize_tail_int8_inplace(subset[name], tail_ratio=tail_ratio)
                    if tail_info is not None:
                        qmeta["tail_mixed"] = tail_info
                        tail_mixed_records[f"model.layers.{i}.{name}"] = tail_info
                quantizers[f"model.layers.{i}.{name}"] = qmeta
                gptq[name].free()
    ...
    return quantizers, tail_mixed_records


def parse_args():
    parser = argparse.ArgumentParser(
        description="Qwen3 GPTQ 4-bit with optional tail mixed precision (tail INT8)."
    )
    parser.add_argument(
        "--model-dir",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="/home/zhou/Documents/yangjia/zip/qwen3_gptq_repro/output/exp_tail_mixed/tail5p_int8_gptq4",
    ...
    parser.add_argument(
        "--enable-tail-mixed",
        action="store_true",
        help="Enable mixed precision on last columns: main GPTQ4 + tail INT8.",
    )
    parser.add_argument(
        "--tail-ratio",
        type=float,
        default=0.05,
        help="Tail columns ratio in [0,1). Example 0.05 means last 5% columns.",
    )
    parser.add_argument(
        "--tail-quant",
        type=str,
        default="int8",
        choices=["int8"],
        help="Tail precision choice. Currently only int8.",
    )
...
def main():
    ...
    if args.enable_tail_mixed and not (0.0 < args.tail_ratio < 1.0):
        raise ValueError("--tail-ratio must be in (0,1) when --enable-tail-mixed is set.")
    ...
    tick = time.time()
    quantizers, tail_mixed_records = qwen3_sequential(
        model=model,
        ...
        enable_tail_mixed=args.enable_tail_mixed,
    ...
    metadata = {
        ...
        "tail_mixed": {
            "enabled": args.enable_tail_mixed,
            "tail_ratio": args.tail_ratio,
            "tail_quant": args.tail_quant,
            "num_layers_with_tail_mixed": len(tail_mixed_records),
            "records": tail_mixed_records,
        },
